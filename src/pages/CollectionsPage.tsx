  ...
  return (
    <div className="mx-auto max-w-2xl px-4 pb-24 pt-6 sm:pb-8">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-slate-900 dark:text-slate-100">Scripture Map</h1>
        <button
          type="button"
          onClick={() => setShowForm((v) => !v)}
          className="rounded-lg bg-blue-700 px-3 py-2 text-sm font-semibold text-white hover:bg-blue-800 dark:bg-blue-600 dark:hover:bg-blue-500"
        >
          + New Collection
        </button>
      </div>
      ...
      {showForm && (
        <form onSubmit={handleCreate} className="mt-4 space-y-2 rounded-xl border border-slate-200 p-4 dark:border-slate-800">
          <input
            value={name}
        ...
        {collections.map((c) => {
          ...
          return (
            <li key={c.id}>
              <Link
                to={`/collections/${c.id}`}
                className="flex items-center justify-between rounded-xl border border-slate-200 bg-white px-4 py-3 shadow-sm hover:shadow-md dark:border-slate-800 dark:bg-slate-900"
              >
                <div>
                ...
                <span className="rounded-full bg-slate-100 px-2 py-1 text-sm font-semibold text-slate-700 dark:bg-slate-800 dark:text-slate-300">
                  {count}
                </span>
              </Link>
            </li>
